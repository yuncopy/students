...
use App\Service\ProposalsService;
...
class ProposalController extends BaseController {
    ...
    public function  getdata(){
        $where = [];
        $data =  ProposalsService::getList($where);
    ...
    public function uniqueName(){
       ...
       $res = ProposalsService::uniqueData($name,$value);
       if($res){
    ...
    public function  addData(){
        if(IsPost()){
            ...
            try{
               ...
               $insertId = ProposalsService::insertData($data);
                ...
                if ($pos_course !== false)  $code = 700900;
                $cont['code'] = $e->getCode();
                
                $insertId = false;
            }
            
            if($insertId){$code = 700200; } //成功
   ...
   public function delData($id){
        try{
            $data = ProposalsService::DelOne($id);  
        } catch ( \Illuminate\Database\QueryException $e){
            $data = false;
        }
        if($data){
           return $this->returnData(700500); 
        }
   }
    ...
    public function updateData(){
         if(IsPost()){
            ...
            //p($dataUpdate);
            try{
                $updateRes =  ProposalsService::updateData($dataUpdate,['id'=>$id]);
            } catch ( \Illuminate\Database\QueryException $e){
