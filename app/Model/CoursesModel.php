...
class CoursesModel extends EloquentModel{
    ...
    public function getList($where){
        ...
        $dataTable = new DataTable(
            ...
            ['id', 'semester_id', 'teacher_id','subject_id','week_id','interval_id','room_id','status','create_time']
        );
        return $dataTable;
    }
    ...
    public function getWhereData($where){
        if($where && is_array($where)){
            $oneObject = $this->where(function($query) use ($where){
                foreach ($where as $key => $value){
                    $query->where($key, $value);
                }
            })->get();
            if($oneObject){
                return $oneObject->toArray();
            }
            return false;
        }
    }
